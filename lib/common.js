...
function wildcardToRegex(str) {
  const regexString = str
    ...
    .map(escapeStringRegexp) // escape the substrings to make sure that they aren't evaluated
    .join(".*"); // construct a regex matching anything at each wildcard location
  return new RegExp(`^${regexString}$`);
}

function matchString(template, str) {
  return template.includes("*")
    ? wildcardToRegex(template).test(str)
    : template === str;
}
