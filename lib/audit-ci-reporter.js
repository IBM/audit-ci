 ...
 *
 * @param {{report: boolean, whitelist: string[], low: boolean, moderate: boolean, high: boolean, critical: boolean}} config
 * `report`: whether to show the NPM audit report in the console. `whitelist`: a list of packages
 * that should not break the build if their vulnerability is found.
 * Only the lowest vulnerability you don't want from [`low`, `moderate`, `high`, `critical`] needs to be set `true`.
 * If `moderate` is set `true` then any `moderate`, `high`, or `critical` vulnerabilities will return rejections.
