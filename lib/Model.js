...
class Model {
  ...
  process(advisory) {
    ...
    this.allowlistedPathsFound.push(
      ...advisory.findings
        ...
        .filter((path) => this.allowlist.paths.some((allowedPath) =>
  ...
  getSummary(advisoryMapper = (a) => a.id) {
    ...
    const allowlistedPathsNotFound = this.allowlist.paths.filter((id) => !this.allowlistedPathsFound.some((foundPath) =>
