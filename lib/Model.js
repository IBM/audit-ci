...
class Model {
  ...
  process(curr) {
    if (!this.failingSverities[curr.severity]) {
      return;
    }

    if (this.whitelistedModuleNames.some(m => m === curr.module_name)) {
      this.summary.whitelistedModulesFound.push(curr.module_name);
      return;
    }

    if (this.whitelistedAdvisotryIds.some(a => Number(a) === curr.id)) {
      this.summary.whitelistedAdvisoriesFound.push(curr.id);
      return;
    }

    this.summary.advisoriesFound.push(curr.id);
    this.foundSeverities.add(curr.severity);
  }
