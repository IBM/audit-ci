...
function yarnSupportsAudit(yarnVersion) {
  return semver.satisfies(yarnVersion, `^${MINIMUM_YARN_CLASSIC_VERSION}`) || semver.gte(yarnVersion, MINIMUM_YARN_BERRY_VERSION);
}
...
function yarnSupportsClassic(yarnVersion) {
  return semver.satisfies(yarnVersion, "^0 || ^1");
}
...
async function audit(config, reporter = reportAudit) {
  ...
  const isYarnClassic = yarnSupportsClassic(yarnVersion);
  ...
  const args = isYarnClassic ? ["audit", "--json"] : ["npm", "audit", "--all", "--recursive", "--json"];
  if (registry) {
