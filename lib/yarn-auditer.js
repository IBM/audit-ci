...
const RETRY_ERROR_MSG = '503 Service Unavailable';
 ...
 */
function audit(config, reporter = reportAudit, attempt = 1) {
  return Promise.resolve()
    ...
    .catch(err => {
      ...
      if (message && message.includes(RETRY_ERROR_MSG) && attempt < 5)
        return audit(config, reporter, attempt + 1);
      if (message && message.includes(RETRY_ERROR_MSG) && attempt < 5)
        return audit(config, reporter, attempt + 1);
      throw err;
    });
